# DeepSeeChat Development Guide

## Project Overview

DeepSeeChat is a bridge that connects AI coding agents (Agent CLI, Codex, Gemini CLI, Cursor, etc.) with messaging platforms (Feishu/Lark, Telegram, Discord, Slack, DingTalk, WeChat Work, QQ, LINE). Users interact with their coding agent through their preferred messaging app.

## Architecture

```
┌─────────────────────────────────────────────────┐
│                   cmd/dsc                │  ← entry point, CLI, daemon
├─────────────────────────────────────────────────┤
...
The `core/` package must remain agnostic. Never write `if p.Name() == "feishu"` or `CreateAgent("dscagent", ...)` in core. Use interfaces and capability checks instead:
...
agent, _ := CreateAgent("dscagent", opts)
...
// In agent/dscagent/
func (a *Agent) CLIBinaryName() string  { return "dsc-agent" }
...
```bash
# Only Agent CLI agent + Feishu and Telegram platforms
make build AGENTS=dscagent PLATFORMS_INCLUDE=feishu,telegram

# Multiple agents
make build AGENTS=dscagent,codex PLATFORMS_INCLUDE=feishu,telegram,discord
```
...
```bash
go build -tags 'no_discord no_dingtalk no_qq no_qqbot no_line' ./cmd/dsc
```

Available tags: `no_acp`, `no_dscagent`, `no_codex`, `no_cursor`, `no_gemini`,
...
4. Create `cmd/dsc/plugin_platform_newplatform.go` with `//go:build !no_newplatform` tag
...
4. Create `cmd/dsc/plugin_agent_newagent.go` with `//go:build !no_newagent` tag
5. Add `newagent` to `ALL_AGENTS` in `Makefile`
6. Optionally implement `AgentDoctorInfo` for `dsc doctor` support
