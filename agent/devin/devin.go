...
//     (parallel to `dscagent`, `cursor`, `codex`, etc.), so minimal
...
//  3. Reports Name() = "devin" so dsc's session store keys,
...
// reads the credentials stored on disk, so dsc never needs to
...
import (
	...
	"github.com/fuguier001/deepseechat/agent/acp"
