...
func TestProcessInteractiveEvents_AppendsReplyFooterWhenEnabled(t *testing.T) {
	...
	agent := &stubReplyFooterAgent{
		...
		workDir: filepath.Join(homeDir, "codes", "dsc"),
		report: &UsageReport{
			Buckets: []UsageBucket{{
				Name: "Rate limit",
				Windows: []UsageWindow{{
	...
	want := "answer\n\n*gpt-5.4 · xhigh · 100% left · ~/codes/dsc*"
	if sent[0] != want {
...
func TestProcessInteractiveEvents_ReplyFooterPrefersSessionRuntimeState(t *testing.T) {
	...
	agentSession.workDir = filepath.Join(homeDir, "codes", "dsc")
	...
	want := "answer\n\n*gpt-5.4 · xhigh · 31% left · ~/codes/dsc*"
	if sent[0] != want {
		t.Fatalf("final reply = %q, want %q", sent[0], want)
	}
}
...
func TestAgentSystemPrompt_MentionsAttachmentSend(t *testing.T) {
	...
	if !strings.Contains(prompt, "dsc send --image") {
		t.Fatalf("prompt missing image send instructions: %q", prompt)
	}
	if !strings.Contains(prompt, "dsc send --file") {
...
func TestCmdHelp_UsesLegacyTextOnPlatformWithoutCardSupport(t *testing.T) {
	...
	if strings.Contains(p.sent[0], "dsc 帮助") {
...
func TestCmdCurrent_UsesLegacyTextOnPlatformWithoutCardSupport(t *testing.T) {
	...
	if strings.Contains(p.sent[0], "dsc") {
...
func TestRenderListCard_MakesEveryVisibleSessionClickable(t *testing.T) {
	...
	// owned-session filter (simulates dsc having created each session).
...
func TestSetupMemoryFile_WritesInstructions(t *testing.T) {
	...
	if !strings.Contains(string(content), "dsc cron add") {
...
func TestSetupMemoryFile_RefreshesLegacyInstructions(t *testing.T) {
	...
	if !strings.Contains(string(content), "dsc send --image") {
...
func TestCmdCronSetup_WritesAndReplies(t *testing.T) {
	...
	if !strings.Contains(p.sent[0], "attachment send-back") {
		t.Errorf("reply = %q, want unified dsc setup success message", p.sent[0])
	}
...
func TestBuildSenderPrompt_Enabled(t *testing.T) {
	...
	expected := "[dsc sender_id=user123 sender_name=\"Alice\" platform=feishu chat_id=channel42]\nhello world"
	if result != expected {
...
func TestBuildSenderPrompt_EmptyUserName(t *testing.T) {
	...
	expected := "[dsc sender_id=user1 platform=feishu chat_id=ch]\nhello"
	if result != expected {
		t.Fatalf("got %q, want %q", result, expected)
	}
}
...
func TestBuildSenderPrompt_NameWithSpaces(t *testing.T) {
	...
	expected := "[dsc sender_id=U999 sender_name=\"Jim Tang\" platform=slack chat_id=C012]\nhi"
	if result != expected {
		t.Fatalf("got %q, want %q", result, expected)
	}
}
...
// filter_external_sessions is enabled, only dsc-tracked sessions
// appear in /list.
...
// by dsc and 1 external. Returns (engine, platform, userKey, agentSessions).
...
// dscLikeSession simulates dsh/gemini/cursor behavior:
...
func newDSCAgentLikeSession(threadID string) *dscLikeSession {
	return &dscLikeSession{
		threadID: threadID,
		events:   make(chan Event, 8),
		alive:    true,
	}
...
func (s *dscLikeSession) Send(prompt string, _ []ImageAttachment, _ []FileAttachment) error {
	s.hasSentID = true
	// dsh sends an early system event with SessionID (empty content)
...
func (s *dscLikeSession) RespondPermission(_ string, _ PermissionResult) error { return nil }
func (s *dscLikeSession) Events() <-chan Event                                 { return s.events }
func (s *dscLikeSession) CurrentSessionID() string {
	if s.hasSentID {
		return s.threadID
	}
	return ""
}
func (s *dscLikeSession) Alive() bool  { return s.alive }
func (s *dscLikeSession) Close() error { s.alive = false; return nil }

// TestSessionName_DSCAgentLikeFlow tests the dsh/gemini/cursor pattern:
// CurrentSessionID()="" initially, but an early EventText carries SessionID.
func TestSessionName_DSCAgentLikeFlow(t *testing.T) {
	sess := newDSCAgentLikeSession("claude-session-001")
	agent := &controllableAgent{nextSession: sess}
	p := &stubPlatformEngine{n: "plain"}
	e := NewEngine("test", agent, []Platform{p}, "", LangEnglish)
	userKey := "test:user1"
	...
	initial := e.sessions.GetOrCreateActive(userKey)
	initial.SetAgentSessionID("claude-session-old", "dsh")
	e.sessions.Save()
	...
	if gotName != "Claude任务" {
		t.Fatalf("GetSessionName(%q) = %q, want %q — dsh-like EventText name mapping failed",
