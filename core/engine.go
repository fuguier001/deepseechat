...
type Engine struct {
	...
	// When true, /list etc. only show sessions tracked by dsc,
...
//
//	[dsc sender_id=ou_abc123 platform=feishu]
//
...
// projectDir is the Agent CLI project directory containing session JSONL files.
...
// AgentTypeName returns the agent type name (e.g. "dsh", "codex").
...
func (e *Engine) getOrCreateWorkspaceAgent(workspace string) (Agent, *SessionManager, error) {
	...
	// already decorated. See dsc#496 and the dsc/core/runas.go
...
func (e *Engine) getOrCreateInteractiveStateWith(sessionKey string, p Platform, replyCtx any, session *Session, sessions *SessionManager, agentOverride Agent, ccSessionKey string) *interactiveState {
	...
	// Inject per-session env vars so the agent subprocess can call `dsc cron add` etc.
...
func (e *Engine) closeAgentSessionWithTimeout(sessionKey string, agentSession AgentSession) {
	...
	// Agent CLI's Stop hooks can take up to 120s (claude-mem uses a
...
// completions in Agent CLI). Events are relayed to the platform immediately.
...
func (e *Engine) handleCommand(p Platform, msg *Message, raw string) bool {
	...
	default:
		...
		// Not a dsc command — notify user, then fall through to agent
...
// returned. When enabled, only sessions tracked by dsc are shown.
...
// filterOwnedSessions removes agent sessions that are not tracked by dsc's
...
// got forwarded verbatim to the agent — and Agent CLI's CLI interprets a
...
const ccConnectInstructionMarker = "<!-- dsc-instructions -->"
...
func (e *Engine) buildSenderPrompt(content, userID, userName, platform, sessionKey string) string {
	...
	if userName != "" {
		...
		return fmt.Sprintf("[dsc sender_id=%s sender_name=\"%s\" platform=%s chat_id=%s]\n%s", userID, safeName, platform, chatID, content)
	}
	return fmt.Sprintf("[dsc sender_id=%s platform=%s chat_id=%s]\n%s", userID, platform, chatID, content)
}
