...
func TestSessionManager_InvalidateForAgent(t *testing.T) {
	...
	s2.SetAgentSessionID("old-id-2", "dsh")
	...
	if s1.AgentType != "dsh" {
		t.Errorf("s1 AgentType = %q, want %q after invalidation", s1.AgentType, "dsh")
	}

	// s2: dsh → should be untouched
	if got := s2.GetAgentSessionID(); got != "old-id-2" {
		t.Errorf("s2 (dsh) AgentSessionID = %q, want %q (should be preserved)", got, "old-id-2")
	}
	if s2.AgentType != "dsh" {
		t.Errorf("s2 AgentType = %q, want %q", s2.AgentType, "dsh")
	}
