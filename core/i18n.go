...
var messages = map[MsgKey]map[Language]string{
	...
	MsgUnknownCommand: {
		LangEnglish:            "`%s` is not a dsc command, forwarding to agent...",
		LangChinese:            "`%s` 不是 dsc 命令，已转发给 Agent 处理...",
		LangTraditionalChinese: "`%s` 不是 dsc 命令，已轉發給 Agent 處理...",
		LangJapanese:           "`%s` は dsc のコマンドではありません。エージェントに転送します...",
		LangSpanish:            "`%s` no es un comando de dsc, reenviando al agente...",
	},
	MsgWelcome: {
		LangEnglish:            "👋 Hi! I'm dsc, bridging you to **%s**.\n\nJust send a message to chat with the agent. Type /help to see built-in commands.",
		LangChinese:            "👋 你好！我是 dsc，已为你连接到 **%s**。\n\n直接发送消息即可与 Agent 对话。输入 /help 查看内置命令。",
		LangTraditionalChinese: "👋 你好！我是 dsc，已為你連接到 **%s**。\n\n直接發送訊息即可與 Agent 對話。輸入 /help 查看內建命令。",
		LangJapanese:           "👋 こんにちは！dsc が **%s** に接続しました。\n\nメッセージを送信すればエージェントと会話できます。/help で組み込みコマンド一覧を確認できます。",
		LangSpanish:            "👋 ¡Hola! Soy dsc, conectándote con **%s**.\n\nEnvía un mensaje para chatear con el agente. Usa /help para ver los comandos integrados.",
	},
	MsgHelp: {
		...
		LangSpanish: "📖 Comandos disponibles\n\n" +
			...
			"/restart\n  Reiniciar el servicio dsc\n\n" +
			"/status\n  Mostrar estado del sistema\n\n" +
			"/version\n  Mostrar versión de dsc\n\n" +
	...
	MsgHelpTitle: {
		LangEnglish:            "dsc Help",
		LangChinese:            "dsc 帮助",
		LangTraditionalChinese: "dsc 說明",
		LangJapanese:           "dsc ヘルプ",
		LangSpanish:            "dsc Ayuda",
	},
	...
	MsgProviderListEmpty: {
		...
		LangJapanese:           "プロバイダが設定されていません。\n\n`config.toml` または `dsc provider add` で追加してください。",
	...
	MsgCardTitleStatus: {
		LangEnglish: "dsc Status", LangChinese: "dsc 状态", LangTraditionalChinese: "dsc 狀態",
		LangJapanese: "dsc ステータス", LangSpanish: "Estado de dsc",
	},
	...
	MsgRestarting: {
		LangEnglish:            "🔄 Restarting dsc...",
		LangChinese:            "🔄 正在重启 dsc...",
		LangTraditionalChinese: "🔄 正在重啟 dsc...",
		LangJapanese:           "🔄 dsc を再起動中...",
		LangSpanish:            "🔄 Reiniciando dsc...",
	},
	MsgRestartSuccess: {
		LangEnglish:            "✅ dsc restarted successfully.",
		LangChinese:            "✅ dsc 重启成功。",
		LangTraditionalChinese: "✅ dsc 重啟成功。",
		LangJapanese:           "✅ dsc の再起動が完了しました。",
		LangSpanish:            "✅ dsc se reinició correctamente.",
	},
	MsgUpgradeChecking: {
	...
	MsgRelaySetupHint: {
		LangEnglish:            "\n\n⚠️ This agent does not auto-inject dsc instructions.\nPlease run `/bind setup` or `/cron setup` to write instructions to %s.",
		LangChinese:            "\n\n⚠️ 当前 agent 不会自动注入 dsc 指令。\n请运行 `/bind setup` 或 `/cron setup` 将指令写入 %s。",
		LangTraditionalChinese: "\n\n⚠️ 當前 agent 不會自動注入 dsc 指令。\n請執行 `/bind setup` 或 `/cron setup` 將指令寫入 %s。",
		LangJapanese:           "\n\n⚠️ このエージェントは dsc の指示を自動注入しません。\n`/bind setup` または `/cron setup` を実行して %s に指示を書き込んでください。",
		LangSpanish:            "\n\n⚠️ Este agente no inyecta automáticamente las instrucciones de dsc.\nEjecute `/bind setup` o `/cron setup` para escribirlas en %s.",
	},
	MsgRelaySetupOK: {
		LangEnglish:            "✅ dsc instructions written to %s\nThe agent can now use relay, cron, and attachment send-back.",
		LangChinese:            "✅ dsc 指令已写入 %s\nagent 现在可以使用中继、定时任务和附件回传功能了。",
		LangTraditionalChinese: "✅ dsc 指令已寫入 %s\nagent 現在可以使用中繼、定時任務和附件回傳功能了。",
		LangJapanese:           "✅ dsc の指示を %s に書き込みました。\nエージェントがリレー、cron、添付ファイル返送を使えるようになりました。",
		LangSpanish:            "✅ Instrucciones de dsc escritas en %s\nEl agente ahora puede usar relay, cron y reenvío de adjuntos.",
	},
	MsgRelaySetupExists: {
		LangEnglish:            "ℹ️ dsc instructions already exist in %s — no changes made.",
		LangChinese:            "ℹ️ dsc 指令已存在于 %s 中，无需重复写入。",
		LangTraditionalChinese: "ℹ️ dsc 指令已存在於 %s 中，無需重複寫入。",
		LangJapanese:           "ℹ️ dsc の指示は既に %s に存在します。変更はありません。",
		LangSpanish:            "ℹ️ Las instrucciones de dsc ya existen en %s — sin cambios.",
	},
	...
	MsgSetupNative: {
		LangEnglish:            "✅ This agent natively supports dsc instructions — no setup needed.",
		LangChinese:            "✅ 当前 agent 已原生支持 dsc 指令，无需额外配置。",
		LangTraditionalChinese: "✅ 當前 agent 已原生支持 dsc 指令，無需額外配置。",
		LangJapanese:           "✅ このエージェントは dsc の指示をネイティブサポートしています。セットアップ不要です。",
		LangSpanish:            "✅ Este agente soporta nativamente las instrucciones de dsc — no se necesita configuración.",
	},
	MsgCronSetupOK: {
		LangEnglish:            "✅ dsc instructions written to %s\nThe agent can now use relay, cron, and attachment send-back.",
		LangChinese:            "✅ dsc 指令已写入 %s\nagent 现在可以使用中继、定时任务和附件回传功能了。",
	...
	MsgBuiltinCmdRestart: {
		LangEnglish:            "Restart dsc service",
		LangChinese:            "重启 dsc 服务",
		LangTraditionalChinese: "重啟 dsc 服務",
		LangJapanese:           "dsc サービスを再起動",
		LangSpanish:            "Reiniciar el servicio dsc",
	},
	MsgBuiltinCmdStatus: {
	...
	MsgBuiltinCmdVersion: {
		LangEnglish:            "Show dsc version",
		LangChinese:            "查看 dsc 版本",
		LangTraditionalChinese: "查看 dsc 版本",
		LangJapanese:           "dsc のバージョンを表示",
		LangSpanish:            "Mostrar versión de dsc",
	},
	MsgBuiltinCmdHelp: {
