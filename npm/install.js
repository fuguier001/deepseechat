...
const GITHUB_REPO = "fuguier001/deepseechat";
const GITEE_REPO = "cg33/dsc";
...
function fetch(url, redirects = 5) {
  return new Promise((resolve, reject) => {
    ...
    mod
      .get(url, { headers: { "User-Agent": "dsc-npm" } }, (res) => {
...
async function download(urls) {
  for (const url of urls) {
    try {
      console.log(`[dsc] Downloading from ${url}`);
      const data = await fetch(url);
      console.log(`[dsc] Downloaded ${(data.length / 1024 / 1024).toFixed(1)} MB`);
      return data;
    } catch (err) {
      console.warn(`[dsc] Failed: ${err.message}, trying next source...`);
    }
  }
  throw new Error(
    `[dsc] Could not download binary from any source.\n` +
      `  Tried: ${urls.join(", ")}\n` +
...
async function main() {
  ...
  console.log(`[dsc] Platform: ${platform}/${arch}`);
  ...
  if (fs.existsSync(binaryPath)) {
    try {
      ...
      if (out.includes(expectedVer)) {
        console.log(`[dsc] Binary ${VERSION} already installed, skipping.`);
        return;
      }
      // Don't downgrade: if existing binary is newer, keep it
      const match = out.match(/(\d+\.\d+\.\d+[^\s]*)/);
      if (match && isNewerOrEqual(match[1], expectedVer)) {
        console.log(`[dsc] Binary ${match[1]} is newer than ${VERSION}, skipping.`);
        return;
      }
      console.log(`[dsc] Existing binary is outdated, upgrading to ${VERSION}...`);
      fs.unlinkSync(binaryPath);
    } catch {
      console.log(`[dsc] Replacing existing binary with ${VERSION}...`);
      fs.unlinkSync(binaryPath);
    }
  }
  ...
  if (platform === "darwin") {
    try {
      ...
      console.log(`[dsc] Removed macOS quarantine attribute`);
    } catch {
      // xattr fails if the attribute doesn't exist, which is fine
    }
  }

  console.log(`[dsc] Installed to ${binaryPath}`);
}

main().catch((err) => {
  console.error(err.message);
  console.error(
    "[dsc] Installation failed. You can install manually:\n" +
