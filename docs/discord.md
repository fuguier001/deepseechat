...
This guide walks you through connecting **dsc** to Discord, so you can chat with your local Agent CLI via a Discord bot.

## Prerequisites

- A Discord account
- A machine that can run dsc (no public IP needed)
- Agent CLI installed and configured
...
2. Enter an application name (e.g. `dsc`)
...
> dsc automatically configures the required Intents (MESSAGE_CONTENT, GUILD_MESSAGES, DIRECT_MESSAGES).
> With `thread_isolation = true`, dsc creates or reuses a Discord thread for each session and routes follow-up messages by thread channel ID.
...
```
level=INFO msg="discord: connected" bot=dsc#0000
...
dsc: 🤔 Thinking...
dsc: 🔧 Tool: Bash(ls -la)
dsc: Here's the project structure...
```
...
│                    Your Local Machine                         │
│                                                              │
│   dsc ◄──► Agent CLI CLI ◄──► Your Project Code    │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
| **Message limit** | 2000 characters per message (auto-split by dsc) |
