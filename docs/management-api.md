# dsc Management API Specification
...
The dsc Management API is an HTTP-based REST API that enables external applications (web dashboards, TUI clients, GUI desktop apps, Mac tray apps) to manage and monitor dsc instances. It complements the existing internal Unix socket API by providing a network-accessible, token-authenticated interface suitable for remote and local management tools.
...
|-----------------------|----------|--------------------------------------------------|
| `version`             | string   | dsc version (e.g. `v1.2.0`)              |
    ...
    "entries": [
      {
        ...
        "attrs": {"socket": "/home/user/.dsc/run/api.sock"}
      }
    ]
  }
}
