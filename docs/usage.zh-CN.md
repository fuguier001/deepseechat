...
- [Agent CLI Router 集成](#dsc-router-集成)
...
dsc provider import --project my-backend  # 从 cc-switch 导入
```
...
dsc weixin setup --project my-project --token '<token>'
...
常用参数：`--api-url`、`--cdn-url`、`--timeout`、`--qr-image`、`--route-tag`、`--bot-type`、`--debug`（详见 `dsc weixin help` 或 [weixin.md](./weixin.md)）。

---

## Agent CLI Router 集成

[Agent CLI Router](https://github.com/musistudio/dsc-router) 可将请求路由到不同模型提供商。

### 安装配置

1. 安装：`npm install -g @musistudio/dsc-router`

2. 配置 `~/.dsc-router/config.json`：
```json
{
...
当 Agent 在本地生成了图片、PDF、日志包、报表等文件，需要把结果直接发回当前聊天时，可以使用 `dsc send` 的附件模式。
...
这两个命令写入的是同一份 dsc 指令。执行任意一个即可。这样 Agent 才会知道：
- 普通文本回复直接正常输出
- 生成附件后用 `dsc send --image/--file` 回传
...
默认值是 `on`。这个开关与 agent 的 `/mode` 独立，只影响 `dsc send --image/--file` 这条图片/文件回传路径。
...
```bash
dsc cron add --cron "0 6 * * *" --prompt "总结 GitHub trending" --desc "每日趋势"
dsc cron list
dsc cron edit <job-id> <field> <value>   # 可改 cron_expr / prompt / enabled / mute / timeout_mins 等
dsc cron del <job-id>
```
...
Agent CLI 会自动创建定时任务。对依赖记忆文件的其他 Agent，先执行一次 `/cron setup` 或 `/bind setup`，效果相同。
...
```
/bind              查看绑定
/bind dscagent   添加 dscagent 项目
/bind gemini       添加 gemini 项目
/bind -dscagent  移除 dscagent
```

### 机器人间通信

```bash
dsc relay send --to gemini "你觉得这个架构怎么样？"
```
...
Bridge 提供 WebSocket + REST 服务，让外部适配器（自定义 UI、机器人、脚本等）可以接入 dsc —— 发送消息、接收 Agent 事件、管理会话。
...
[projects.agent]
type = "dscagent"  # 或 codex, cursor, gemini, qoder, opencode, iflow
