...
This guide walks you through connecting **dsc** to Telegram, so you can chat with your local Agent CLI via a Telegram bot.
...
Enter a **display name** for your bot (e.g. `dsc`).
...
Telegram topics include a `message_thread_id`. dsc uses that thread ID
...
3. Have you sent a message after starting dsc? (The bot only receives messages after startup)
