...
Complete guide to using dsc features.
...
- [Agent CLI Router Integration](#dsc-router-integration)
...
dsc rotates to a fresh session automatically after long inactivity:
...
```bash
dsc provider add --project my-backend --name relay --api-key sk-xxx --base-url https://api.relay.com
dsc provider list --project my-backend
dsc provider remove --project my-backend --name relay
dsc provider import --project my-backend  # from cc-switch
```
...
|-------|-----------|------------|
| Agent CLI | `ANTHROPIC_API_KEY` | `ANTHROPIC_BASE_URL` |
...
> **Agent support**: Agent CLI today. Other agents fall back to the
> supervisor user; see the tracking issue for migration status.

### What this is

By default, every agent session dsc spawns runs as the same Unix
user that runs `dsc` itself. If an agent misbehaves — reads a
...
dsc spawns that project's agent command via
...
```
# /etc/sudoers.d/dsc (install with `sudo visudo -f ...`)
...
If that command succeeds, dsc will refuse to start. Remove any
...
dsc refuses to start if the target user cannot read+write the
`work_dir` root, and warns (non-fatal) for descendant paths that look
inaccessible.

#### 5. Audit the setup before starting dsc

```bash
dsc doctor user-isolation
```

This runs the full preflight (the three go/no-go gates from
[#496](https://github.com/fuguier001/deepseechat/issues/496)) and an
...
`~/.dsc/audits/<timestamp>-<project>.json`.
...
```bash
dsc doctor user-isolation --print-script
```
...
- [ ] **Claude OAuth credentials** — if you authenticate via `claude.ai`
      ...
      dsc spawns. The recommended fix is to symlink the target
...
- [ ] **Shell profile** — `~/.profile` / `~/.bashrc` on the target user
      ...
      with `sudo -iu partseeker-coder` before wiring dsc.

After migration, run `dsc doctor user-isolation` again. The
...
- **"passwordless sudo to user X is not configured"** — step 2 of setup
  ...
  the rule, run `visudo -c` to validate syntax, then restart dsc.
...
- **"descendant scan timed out"** — non-fatal. The `work_dir` is large
  ...
  `dsc doctor user-isolation` manually if you want the full
...
dsc weixin bind --project my-project --token '<token>'
dsc weixin new --project my-project
```
...
- See `dsc weixin help` for flags (`--api-url`, `--cdn-url`, `--route-tag`, etc.).

---

## Agent CLI Router Integration

[Agent CLI Router](https://github.com/musistudio/dsc-router) routes requests to different model providers.

### Setup

1. Install: `npm install -g @musistudio/dsc-router`

2. Configure `~/.dsc-router/config.json`:
```json
{
...
Send voice messages — dsc transcribes them automatically.
...
When an agent generates a local image, PDF, report, bundle, or other file and needs to deliver it directly to the current chat, use attachment mode in `dsc send`.
...
These two commands write the same dsc instructions. Either one is enough. After that, the agent knows:
- normal text replies should be returned normally
- generated attachments should be sent back with `dsc send --image/--file`
...
The default is `on`. This switch is independent from the agent's `/mode` and only affects `dsc send --image/--file`.

### CLI examples

```bash
dsc send --image /absolute/path/to/chart.png
dsc send --file /absolute/path/to/report.pdf
dsc send --file /absolute/path/to/report.pdf --image /absolute/path/to/chart.png
```
...
- There must be an active session; otherwise the command fails because dsc has no chat context to deliver to.
...
```bash
dsc cron add --cron "0 6 * * *" --prompt "Summarize GitHub trending" --desc "Daily Trending"
dsc cron list
dsc cron edit <job-id> <field> <value>   # e.g. cron_expr, prompt, enabled, mute, timeout_mins
dsc cron del <job-id>
```
...
Agent CLI auto-creates the cron job. For other agents that rely on memory files, run `/cron setup` or `/bind setup` once first; both write the same instructions.
...
```
/bind              Show bindings
/bind dscagent   Add dscagent project
/bind gemini       Add gemini project
/bind -dscagent  Remove dscagent
```

### Bot-to-Bot Communication

```bash
dsc relay send --to gemini "What do you think about this architecture?"
```
...
| `POST` | `/api/v1/restart` | Restart dsc |
...
The Bridge exposes a WebSocket + REST server so external adapters (custom UIs, bots, scripts) can interact with dsc sessions — send messages, receive events, manage sessions.
