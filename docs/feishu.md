...
本文档介绍如何将 **dsc** 接入飞书，让你可以通过飞书机器人远程调用 Agent CLI。

## 前置要求

- 飞书账号（个人或企业均可）
- 一台可运行 dsc 的设备（无需公网 IP）
- Agent CLI 已安装并配置完成
...
如果你已经装好 `dsc`，可以直接用内置命令完成“新建机器人/关联已有机器人”，并自动写回 `config.toml`：
...
> ⚠️ **重要**：请妥善保存这两个凭证，后续配置 dsc 时需要用到。App Secret 只会显示一次，如果忘记了需要重置。

### 2.3 配置到 dsc

将凭证配置到 dsc 的 `config.toml` 中：
...
启动后，dsc 会自动与飞书建立 WebSocket 长连接。你会在日志中看到：
...
dsc 默认使用交互卡片显示权限确认、provider 选择等操作。如果点击按钮后无响应、显示加载超时或报错，请检查：
...
通常是飞书注册模板侧的展示文案，不影响返回 `app_id/app_secret` 和接入 dsc。
