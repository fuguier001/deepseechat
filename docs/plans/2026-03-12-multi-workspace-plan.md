...
**Goal:** Enable a single dsc bot to serve multiple workspaces, routing messages to different Agent CLI sessions based on Slack channel.
...
And in `agent/dscagent/dscagent.go`:
```go
...
func (e *Engine) getOrCreateWorkspaceAgent(workspace string) (Agent, *SessionManager, error) {
	...
	// Copy relevant options from the original agent if it's a dscagent agent
	...
	agent, err := CreateAgent("dscagent", agentOpts)
	if err != nil {
...
```bash
git add core/engine.go core/interfaces.go core/workspace_state.go agent/dscagent/dscagent.go
...
**Files:**
- Modify: `cmd/dsc/main.go` (~line 139 project setup loop)
