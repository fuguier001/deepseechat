...
**Tech Stack:** Go, Agent CLI CLI (stream-json protocol), slog structured logging
...
- Modify: `agent/dscagent/session.go:268-282` — parse usage from result JSON

**Step 1: Write failing test**

Add to a new file or existing test for dscagent session parsing. Since `handleResult` is on the unexported `claudeSession`, test via the event channel:

Create `agent/dscagent/session_test.go` test (or add to existing):
...
Run: `go test ./agent/dscagent/ -run TestHandleResultParsesUsage -v`
...
In `agent/dscagent/session.go`, modify `handleResult`:
...
```bash
git add core/message.go agent/dscagent/session.go agent/dscagent/session_test.go
git commit -m "feat: parse token usage from Agent CLI result events"
```
...
- Modify: `agent/dscagent/session.go:42-117` — spawn logging with JSONL path/size
- Modify: `agent/dscagent/dscagent.go:195-224` — log cwd at StartSession
...
**Step 2: Add JSONL file size logging in `dscagent.StartSession`**

In `agent/dscagent/dscagent.go`, in `StartSession`, before calling `newClaudeSession`, add:
...
In `agent/dscagent/session.go`, the `readLoop` already logs stderr on failure (line 125). Enhance it:
...
Run: `go test ./core/ ./agent/dscagent/ -v -count=1`
Expected: PASS

**Step 6: Commit**

```bash
git add core/engine.go agent/dscagent/session.go agent/dscagent/dscagent.go
