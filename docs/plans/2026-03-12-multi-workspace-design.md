...
Enable a single dsc bot (one Slack token) to serve multiple workspaces, with the channel determining which Agent CLI working directory and session to use.
...
Persisted in `~/.dsc/workspace_bindings.json`:
...
1. **Spawn on first message** — start a Agent CLI subprocess with `work_dir` set to the resolved workspace.
