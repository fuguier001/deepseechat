...
The Bridge Protocol allows **external platform adapters** written in any programming language to connect to dsc at runtime via WebSocket. This eliminates the requirement to write Go code and recompile the binary for every new platform integration.
...
The `BridgePlatform` is a built-in platform inside dsc that:
...
| `reply_ctx` | string | yes | Opaque context string the adapter needs to route replies back. dsc echoes this in every reply. |
...
Keepalive. dsc responds with `pong`.
...
Send a structured card to the user. Only sent if the adapter declared `"card"` capability; otherwise dsc falls back to `reply` with `card.RenderText()`.
...
If a capability is not declared, dsc will automatically degrade:
...
3. Resume normal operation — dsc maintains session state independently of the connection.

### Message Ordering

Messages within a single WebSocket connection are ordered. dsc processes adapter messages sequentially per session key.

### Timeouts

- **Ping interval**: Adapters should send `ping` at least every 30 seconds.
- **Connection timeout**: dsc closes idle connections after 90 seconds without a ping.
- **Reply timeout**: If an agent takes too long, dsc may send an error reply. The adapter does not need to handle this specially.
...
1. **Keep it stateless** — the adapter should be a thin translation layer. All session state lives in dsc.
...
The protocol version is declared in the `register` message via `metadata.protocol_version`. The current version is `1`. dsc will reject connections with incompatible versions and respond with a `register_ack` containing an error.
