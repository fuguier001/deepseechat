...
func TestBuildWindowsTaskScript(t *testing.T) {
	cfg := Config{
		BinaryPath: `C:\Program Files\dsc\dsc.exe`,
		WorkDir:    `C:\Users\me\.dsc`,
		LogFile:    `C:\Users\me\.dsc\logs\dsc.log`,
	...
	for _, want := range []string{
		`$env:CC_LOG_FILE = 'C:\Users\me\.dsc\logs\dsc.log'`,
		`$env:CC_LOG_MAX_SIZE = '10485760'`,
		`$env:PATH = 'C:\Program Files\nodejs;C:\Users\me\AppData\Local\Programs'`,
		`$env:HTTPS_PROXY = 'http://127.0.0.1:7890'`,
		`$env:http_proxy = 'http://127.0.0.1:7890'`,
		`Set-Location -LiteralPath 'C:\Users\me\.dsc'`,
		`while ($true) {`,
...
func TestWindowsTaskActionRunsHidden(t *testing.T) {
	got := windowsTaskAction(`C:\Users\me\.dsc\dsc-daemon.ps1`)
	for _, want := range []string{
		`powershell.exe`,
		`-WindowStyle Hidden`,
		`-NoProfile`,
		...
		`-File "C:\Users\me\.dsc\dsc-daemon.ps1"`,
	} {
...
func TestWindowsTaskCreateUsesLimitedInteractivePrincipal(t *testing.T) {
	...
	if err := createWindowsTask(`C:\Users\me\.dsc\dsc-daemon.ps1`); err != nil {
...
func TestWindowsTaskMatchesActionRequiresExactAction(t *testing.T) {
	...
	if !windowsTaskMatchesAction(`C:\Users\me\.dsc\dsc-daemon.ps1`) {
		t.Fatal("windowsTaskMatchesAction() = false, want true")
	}
	for _, want := range []string{
		`$expectedArgs = '-WindowStyle Hidden -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "C:\Users\me\.dsc\dsc-daemon.ps1"'`,
...
func TestPowerShellLiteralEscapesSingleQuotes(t *testing.T) {
	got := powerShellLiteral(`C:\Users\O'Brien\.dsc`)
	want := `'C:\Users\O''Brien\.dsc'`
	if got != want {
