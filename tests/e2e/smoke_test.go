...
// Package e2e contains smoke and regression tests for dsc.
...
import (
	...
	"github.com/fuguier001/deepseechat/tests/mocks"
	"github.com/fuguier001/deepseechat/tests/mocks/fake"
...
`
	...
	assert.Equal(t, "~/.dsc", cfg.DataDir)
	assert.Len(t, cfg.Projects, 1)
	assert.Equal(t, "test-project", cfg.Projects[0].Name)
	assert.Equal(t, "dscagent", cfg.Projects[0].Agent.Type)
