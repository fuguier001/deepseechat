...
import (
	...
	"github.com/fuguier001/deepseechat/agent/dscagent"
	"github.com/fuguier001/deepseechat/agent/codex"
	"github.com/fuguier001/deepseechat/agent/cursor"
	"github.com/fuguier001/deepseechat/agent/gemini"
	"github.com/fuguier001/deepseechat/agent/opencode"
...
func TestNewSession_DSCAgent(t *testing.T) {
	t.Parallel()
	e, mp, _, cleanup := setupIntegrationEngine(t, "dscagent")
	defer cleanup()
...
func TestSharedCasesAcrossAgents(t *testing.T) {
	agents := []string{"dscagent", "codex", "cursor", "gemini", "opencode"}
...
// Note: Agent CLI has workspace-level memory (CLAUDE.md) that persists
...
func TestConcurrentSessionIsolation(t *testing.T) {
	...
	e1, mp1, _, cleanup1 := setupIntegrationEngine(t, "dscagent")
	defer cleanup1()
	e2, mp2, _, cleanup2 := setupIntegrationEngine(t, "dscagent")
	defer cleanup2()
