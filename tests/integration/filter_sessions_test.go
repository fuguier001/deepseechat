...
// writeDSCAgentSessionFixture creates a realistic Agent CLI JSONL session file.
func writeDSCAgentSessionFixture(t *testing.T, projectDir, sessionID, userPrompt string) {
	t.Helper()
...
// wires it into a real Engine. Some sessions are tracked by dsc (via
...
func setupFilterSessionTest(t *testing.T, agentType string, filterEnabled bool) (
	...
	case "dscagent":
		...
		for i, id := range allIDs {
			writeDSCAgentSessionFixture(t, projectDir, id, fmt.Sprintf("Prompt for session %d", i+1))
...
// ---------------------------------------------------------------------------
// Agent CLI: real agent adapter + Engine filter integration
// ---------------------------------------------------------------------------

func TestRealDSCAgent_FilterDisabled_ListShowsAll(t *testing.T) {
	skipUnlessBinaryAvailable(t, "dscagent")
	e, mp, userKey, _, _ := setupFilterSessionTest(t, "dscagent", false)
	defer e.Stop()
...
func TestRealDSCAgent_FilterEnabled_ListHidesExternal(t *testing.T) {
	skipUnlessBinaryAvailable(t, "dscagent")
	e, mp, userKey, _, _ := setupFilterSessionTest(t, "dscagent", true)
	defer e.Stop()
...
func TestRealDSCAgent_FilterEnabled_SwitchExternal_Rejected(t *testing.T) {
	skipUnlessBinaryAvailable(t, "dscagent")
	e, mp, userKey, _, externalIDs := setupFilterSessionTest(t, "dscagent", true)
	defer e.Stop()
