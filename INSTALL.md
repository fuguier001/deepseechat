# dsc Installation & Configuration Guide

> **This document is designed to be read by AI coding agents (Agent CLI, Cursor, Gemini CLI, etc.) to help users install and configure dsc.** You can feed this file directly to your AI agent.

## What is dsc?

dsc bridges local AI coding assistants to messaging platforms (Feishu, DingTalk, Telegram, Slack, Discord, LINE, WeChat Work, QQ, **personal WeChat** via the **Weixin (ilink)** platform).

GitHub: https://github.com/fuguier001/deepseechat

**IMPORTANT: If you are an AI agent helping a user install dsc, use interactive tools (like AskUserQuestion) to guide them through configuration choices:**
- Agent selection (Agent CLI, Cursor, Gemini, etc.)
...
After installation, the `dsc` binary will be available globally.
...
Go to https://github.com/fuguier001/deepseechat/releases and download the binary for your platform.

Typical artifact names (check the release page for exact filenames):

- Linux: `dsc-<version>-linux-amd64` (or `.tar.gz`)
- macOS: `dsc-<version>-darwin-amd64` / `arm64`
- Windows: `dsc-<version>-windows-amd64.exe` (or `.zip`)

```bash
# Example for Linux amd64 (replace URL with the asset link from the release you chose):
curl -L -o dsc https://github.com/fuguier001/deepseechat/releases/latest/download/dsc-linux-amd64
chmod +x dsc
...
```bash
xattr -d com.apple.quarantine dsc
```
...
```bash
git clone https://github.com/fuguier001/deepseechat.git
cd dsc
make build
# Binary will be at ./dsc
```

## Step 2: Install your AI Agent

dsc supports multiple local coding agents. Install at least one:

```bash
# Agent CLI
npm install -g @anthropic-ai/dsc
...
> **💡 Recommended: Use the Web UI** — After installing, run `dsc web` to configure the web admin and open the dashboard in your browser. You can visually create projects, add platforms, manage API providers, and even chat with your agent directly from the browser — no need to edit TOML files by hand. **Note:** `dsc web` only configures and opens the browser — you still need to run `dsc` separately to start the service.

If you prefer manual configuration, dsc looks for config in this order:
1. `-config <path>` flag (explicit)
2. `./config.toml` (current directory)
3. `~/.dsc/config.toml` (global, **recommended**)

If no config file exists, running `dsc` will auto-create a starter template at `~/.dsc/config.toml`.
...
cp config.example.toml ~/.dsc/config.toml
# Or just run dsc once — it will create a starter config automatically
```
...
[projects.agent]
type = "dscagent"  # or "codex", "cursor", "gemini", "qoder", "opencode", "iflow"

[projects.agent.options]
work_dir = "/absolute/path/to/your/project"
mode = "default"

# --- Agent CLI mode options ---
...
dsc feishu setup --project my-project
dsc feishu setup --project my-project --app cli_xxx:sec_xxx

# Force modes (usually unnecessary)
dsc feishu new --project my-project

dsc feishu bind --project my-project --app cli_xxx:sec_xxx
```
...
- If `--project` does not exist, dsc creates it automatically.
...
4. In the app → **Receive Messages** → Set API Receive:
   ...
   - **Start dsc FIRST, then save** (to pass URL verification)
...
3. Restart dsc, then send a message from WeChat once so `context_token` is cached.

If you already have a Bearer token, use `dsc weixin bind --project my-project --token '<token>'`.
...
```bash
dsc web    # configure web admin & open browser (does NOT start dsc)
dsc        # start the service
```

> **Note:** `dsc web` only configures the web admin and opens the dashboard in your browser — it does **not** start the dsc service itself. You still need to run `dsc` (or `dsc --config <path>`) separately to actually start the bridge. Think of it as two steps: configure first, then run.

```

Alternatively, open a **separate terminal** and run dsc there — this avoids the issue entirely.
...
dsc -config /path/to/config.toml
...
level=INFO msg="engine started" project=my-project agent=dscagent platforms=1
level=INFO msg="dsc is running" projects=1
```
...
During a session, the agent may ask for tool permissions. Reply:
...
## Step 7: Enable Natural Language Scheduling (通用 Agent)

dsc supports scheduled tasks (cron jobs). You can always create them via slash commands (`/cron add ...`) or CLI (`dsc cron add ...`), but to let the agent **understand natural language** like "every day at 6am, summarize trending repos", the agent needs to know about dsc's cron CLI.

**Agent CLI** handles this automatically via `--append-system-prompt` — no extra setup needed.
...
This project is managed via dsc, a bridge to messaging platforms.
...
"every Monday morning"), use the Bash/shell tool to run:
  ...
  dsc cron add --cron "<min> <hour> <day> <month> <weekday>" --prompt "<task description>" --desc "<short label>"

Environment variables CC_PROJECT and CC_SESSION_KEY are already set — do NOT
specify --project or --session-key.

Examples:
  dsc cron add --cron "0 6 * * *" --prompt "Collect GitHub trending repos and send a summary" --desc "Daily GitHub Trending"
  dsc cron add --cron "0 9 * * 1" --prompt "Generate a weekly project status report" --desc "Weekly Report"

To list, edit, or delete cron jobs:
  dsc cron list
  dsc cron edit <job-id> <field> <value>
  dsc cron del <job-id>
...
Run `dsc cron edit --help` for the full field list.

Examples:
  dsc cron edit abc123 cron_expr "0 9 * * *"
  dsc cron edit abc123 enabled false
  dsc cron edit abc123 prompt "Updated daily summary task"

## Send message to current chat
To proactively send a message back to the user's chat session (use --stdin heredoc for long/multi-line messages):

  dsc send --stdin <<'CCEOF'
  your message here (any special characters are safe)
  CCEOF

For short single-line messages:

  dsc send -m "short message"
```

After adding this file, the agent will be able to translate natural language scheduling requests into `dsc cron add` commands automatically.

> **Tip:** You may want to add `AGENTS.md` / `.cursorrules` / `GEMINI.md` to your `.gitignore` if you don't want dsc instructions committed to version control.

## Multi-Project Setup

A single dsc process can manage multiple projects. Each project has its own agent, work directory, and platforms:
...
Check the latest release at https://github.com/fuguier001/deepseechat/releases and compare with your local version. To upgrade:

```bash
# Linux/macOS — replace with your platform suffix
curl -L -o /usr/local/bin/dsc https://github.com/fuguier001/deepseechat/releases/latest/download/dsc-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m | sed 's/x86_64/amd64/' | sed 's/aarch64/arm64/')
...
After upgrading, restart the running dsc process.

## Step 8: Run as Background Service (Optional)

You can run dsc as a daemon managed by the OS init system (Linux systemd user service, macOS launchd LaunchAgent, Windows Task Scheduler task).

### Install the daemon

```bash
dsc daemon install --config ~/.dsc/config.toml
```

You can also point the daemon at the directory that contains `config.toml`:

```bash
dsc daemon install --work-dir ~/.dsc
```
...
```bash
dsc daemon logs           # tail current log
dsc daemon logs -f         # follow (like tail -f)
dsc daemon logs -n 100     # last 100 lines
dsc daemon logs --log-file /path/to/log  # custom log file
```

Logs auto-rotate at the configured max size and keep one backup.

On Windows, `daemon install` creates a native Task Scheduler task named `dsc`.
The task runs at user logon and is also started immediately after installation. The
installer writes a small PowerShell launcher under `~/.dsc` so the scheduled
task uses the selected config directory, log file, PATH, and proxy environment.

### Uninstall

```bash
dsc daemon uninstall
```
...
- **Image Messages**: Send images to Agent CLI for multimodal analysis
- **API Provider Management**: Runtime switching between API providers via `/provider` command or CLI
- **CLI Send**: `dsc send` to inject messages into active sessions from external processes

## Troubleshooting

- **"session already in use"** — A previous Agent CLI process may still be running. Use `/new` to start a fresh session.
- **No response from bot** — Check `dsc` logs. Set `level = "debug"` in `[log]` for verbose output.
...
- **macOS binary won't open** — Run `xattr -d com.apple.quarantine dsc` to remove quarantine flag.
