...
func printProviderUsage() {
	fmt.Println(`Usage: dsc provider <command> [options]
...
Examples:
  dsc provider add --project my-backend --name relay --api-key sk-xxx
  dsc provider add --project my-backend --name bedrock --env CLAUDE_CODE_USE_BEDROCK=1,AWS_PROFILE=bedrock
  dsc provider list --project my-backend
  dsc provider remove --project my-backend --name relay
  dsc provider import --project my-backend
  dsc provider presets
  dsc provider global list
  dsc provider global add --name minimaxi --api-key sk-xxx --base-url https://api.minimaxi.chat/v1`)
}
...
func runProviderGlobal(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, `Usage: dsc provider global <command>
...
func runGlobalProviderList(args []string) {
	...
	if len(providers) == 0 {
		...
		fmt.Println("\nAdd one with: dsc provider global add --name <name> --api-key <key>")
		return
	}
