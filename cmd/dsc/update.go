...
const (
	githubRepo   = "fuguier001/deepseechat"
	...
	giteeAPI     = "https://gitee.com/api/v5/repos/cg33/dsc/releases/latest"
)
...
func getUpdateHintIfAvailable() string {
	...
	if isNewer(cachedVer, version) {
		return fmt.Sprintf("\n📦 Update available: %s → %s  (run: dsc update)\n", version, cachedVer)
	}
	return ""
}
...
func runUpdate() {
	...
	fmt.Println("Restart dsc to use the new version.")
}
...
func binaryAssetName(tag string) string {
	...
	name := fmt.Sprintf("dsc-%s-%s-%s", tag, goos, goarch)
	if goos == "windows" {
		name += ".exe"
	}
	return name
...
func archiveAssetName(tag string) string {
	goos := runtime.GOOS
	goarch := runtime.GOARCH
	base := fmt.Sprintf("dsc-%s-%s-%s", tag, goos, goarch)
	if goos == "windows" {
		return base + ".zip"
	}
	return base + ".tar.gz"
...
// extractBinaryFromArchive extracts the dsc binary from a .tar.gz or .zip archive.
...
func extractFromTarGz(archivePath string) (string, error) {
	...
	for {
		...
		if strings.HasPrefix(hdr.Name, "dsc") {
			tmp, err := os.CreateTemp("", "dsc-update-*")
			if err != nil {
				return "", err
			}
...
func extractFromZip(archivePath string) (string, error) {
	...
	for _, f := range r.File {
		if !strings.HasPrefix(f.Name, "dsc") {
			continue
		}
		rc, err := f.Open()
		if err != nil {
...
// (node_modules/dsc/bin/) and updates the package.json version to
...
func syncNpmPackageVersion(execPath, newVer string) {
	...
	if err := os.WriteFile(pkgJSON, out, 0o644); err != nil {
		...
		fmt.Println("   please run: npm update -g dsc")
	} else {
