...
func printAgentSIDUsage() {
	fmt.Println(`Usage: dsc agent-sid [options]

Print the agent session ID (e.g. Agent CLI, Codex, Gemini CLI) for the
current session. This is the ID used for --resume.

The command reads from the persisted session file; no running dsc
...
Examples:
  dsc agent-sid                         Auto-detect from env (inside a session)
  dsc agent-sid -p mybot -s "discord:123:456"`)
}
