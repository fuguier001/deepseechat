...
func printRelayUsage() {
	fmt.Println(`Usage: dsc relay <command> [options]

Commands:
  send      Send a message to another bot via relay

Run 'dsc relay <command> --help' for details.`)
}

func printRelaySendUsage() {
	fmt.Println(`Usage: dsc relay send [options] [<target_project> <message>]
...
Examples:
  dsc relay send --to claude-bot "What's the weather today?"
  dsc relay send claude-bot What is the weather today`)
}
