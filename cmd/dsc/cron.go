...
func runCronAdd(args []string) {
	...
	// Fallback to env vars (set by dsc when spawning agent)
	if project == "" {
	...
	if _, err := os.Stat(sockPath); os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "Error: dsc is not running (socket not found: %s)\n", sockPath)
		os.Exit(1)
	}
...
func runCronInfo(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Usage: dsc cron info <id> [field]")
		fmt.Fprintln(os.Stderr, "Use 'dsc cron list' to see all task IDs.")
		os.Exit(1)
	}
...
func runCronEdit(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Run 'dsc cron edit --help' for usage.")
		os.Exit(1)
	}
	if field == "" {
...
func printCronUsage() {
	fmt.Println(`Usage: dsc cron <command> [options]
...
Run 'dsc cron <command> --help' for details.`)
}

func printCronAddUsage() {
	fmt.Println(`Usage: dsc cron add [options] [<min> <hour> <day> <month> <weekday> <prompt>]
...
Options:
  ...
  -c, --cron <expr>          Cron expression, e.g. "0 6 * * *"
      ...
      --data-dir <path>      Data directory (default: ~/.dsc)
  -h, --help                 Show this help

Examples:
  dsc cron add --cron "0 6 * * *" --prompt "Collect GitHub trending data" --desc "Daily Trending"
  dsc cron add --cron "*/30 * * * *" --exec "df -h" --desc "Disk usage check"
  dsc cron add 0 6 * * * Collect GitHub trending data and send me a summary`)
}

func printCronEditUsage() {
	fmt.Println(`Usage: dsc cron edit <id> <field> <value> [options]
...
Examples:
  ...
  dsc cron edit abc123 description "Daily standup reminder"
  dsc cron edit abc123 timeout_mins 60
  dsc cron edit abc123 mute true`)
}
