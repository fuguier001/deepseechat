...
func runSessions(args []string) {
	...
	case "show":
		...
		if id == "" {
			...
			fmt.Fprintln(os.Stderr, "Usage: dsc sessions show <session-id> [-n N]")
			os.Exit(1)
		}
...
func runSessionsShow(dataDir, id string, limit int) {
	...
	if record == nil {
		...
		fmt.Fprintln(os.Stderr, "Use 'dsc sessions list' to see available sessions.")
		os.Exit(1)
	}
...
func printSessionsUsage() {
	fmt.Println(`Usage: dsc sessions [command] [options]
...
Examples:
  dsc sessions                           Interactive TUI browser
  dsc sessions list                      List all sessions
  dsc sessions show "mybot:s1"           Show all messages in session
  dsc sessions show "#1" -n 20           Show last 20 messages of first session`)
}
