...
func resolveSocketPath(dataDir string) string {
	...
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".dsc", "run", "api.sock")
	}
	return filepath.Join(".dsc", "run", "api.sock")
}

func printSendUsage() {
	fmt.Println(`Usage: dsc send [options] <message>
       dsc send [options] -m <message>
       dsc send [options] --stdin < file
       dsc send [options] --image <path>
       dsc send [options] --file <path>
       echo "msg" | dsc send [options] --stdin

Send a message or attachment to an active dsc session.
...
Examples:
  dsc send "Daily summary: ..."
  dsc send -m "Build completed successfully"
  dsc send --message "Chart generated" --image /tmp/chart.png
  dsc send --file /tmp/report.pdf
  dsc send --stdin <<'EOF'
