...
type Config struct {
	DataDir        string `toml:"data_dir"` // session store directory, default ~/.dsc
...
// ObserveConfig controls forwarding of native terminal Agent CLI sessions to a messaging platform.
...
type ProjectConfig struct {
	...
	// ResetOnIdleMins automatically rotates to a new dsc session after
	...
	// runs dsc itself. Requires passwordless sudo to the target user
	...
	// dsc, hiding sessions created by direct CLI usage in the same work_dir.
...
type ProviderConfig struct {
	...
	AgentTypes      []string                         `toml:"agent_types,omitempty"`       // optional: restrict to specific agent types (e.g. ["dscagent", "codex"])
...
func Load(path string) (*Config, error) {
	...
	if cfg.DataDir == "" {
		if home, err := os.UserHomeDir(); err == nil {
			cfg.DataDir = filepath.Join(home, ".dsc")
		} else {
			cfg.DataDir = ".dsc"
		}
	}
...
func patchProjectAgentOption(projectName, key, value string) error {
	...
	if projSpan.agentOptionsStart < 0 {
		...
		if projSpan.agentStart < 0 {
			...
			block := []string{"", "[projects.agent]", "type = \"dscagent\"", "", "[projects.agent.options]"}
			lines = insertLines(lines, insertAt, block)
		} else {
