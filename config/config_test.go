				...
				Projects: []ProjectConfig{
					func() ProjectConfig {
						...
						p.References = ReferenceConfig{
							NormalizeAgents: []string{"codex", "dscagent"},
...
func TestPickAgentTemplateForNewProject(t *testing.T) {
	...
	t.Run("clone from existing project", func(t *testing.T) {
		...
		if got.Type != "dscagent" {
			t.Errorf("Type = %q, want dscagent", got.Type)
		}
...
func TestResolveProviderRefs_AgentTypeFiltering(t *testing.T) {
	cfg := &Config{
		Providers: []ProviderConfig{
			{Name: "claude-only", APIKey: "key-c", AgentTypes: []string{"dscagent"}},
	...
	// dscagent project: gets claude-only + universal, skips codex-only
	p0 := cfg.Projects[0].Agent.Providers
	if len(p0) != 2 {
...
func TestResolveProviderRefs_AgentTypesFilter(t *testing.T) {
	cfg := &Config{
		Providers: []ProviderConfig{
			{Name: "claude-only", AgentTypes: []string{"dscagent"}},
...
func TestResolveProviderRefs_EndpointsOverride(t *testing.T) {
	...
	// dscagent project: should keep original base_url and model
...
func TestResolveProviderRefs_SplitProviderPattern(t *testing.T) {
		...
		Providers: []ProviderConfig{
			{
				...
				AgentTypes: []string{"dscagent", "gemini"},
	...
	// dscagent project should only get "ssy" (not ssy-codex)
	cp := cfg.Projects[0].Agent.Providers
